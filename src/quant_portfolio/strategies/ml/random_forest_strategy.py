"""Random Forest Strategy - RF classification signals."""
...
class RandomForestStrategy(Strategy):
    ...
    Trains a random forest on technical features to predict
    next-day return direction.

    Parameters
    ----------
    lookback : int
        Training lookback window (default 120).
    n_estimators : int
    ...
    def _build_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        features["vol_ratio"] = volume / (volume.rolling(20).mean() + 1e-10)
